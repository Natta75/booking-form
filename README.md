...
- 🛡️ **reCAPTCHA Enterprise** - современная защита от спама и ботов (Google Cloud)
...
│   ├── recaptcha-enterprise.js    # reCAPTCHA Enterprise интеграция
...
│   └── RECAPTCHA_ENTERPRISE_SETUP.md # Настройка reCAPTCHA Enterprise
...
# reCAPTCHA Enterprise (новая система Google Cloud)
...
**⚠️ ВАЖНО:** Проект использует **reCAPTCHA Enterprise** - новую систему Google Cloud, которая заменила устаревший reCAPTCHA v3.

**📚 Подробная инструкция:**
См. [docs/RECAPTCHA_ENTERPRISE_SETUP.md](docs/RECAPTCHA_ENTERPRISE_SETUP.md) - полное руководство по настройке с скриншотами и решением проблем.

**Краткая версия:**
1. Создайте проект в [Google Cloud Console](https://console.cloud.google.com/)
2. Включите **reCAPTCHA Enterprise API**
3. Создайте ключ reCAPTCHA (тип: Website, score-based)
4. Создайте **API Key** с доступом к reCAPTCHA Enterprise API
5. Подтвердите домен в Google Cloud
6. Заполните переменные в `.env`:
   - `RECAPTCHA_SITE_KEY` - из reCAPTCHA Enterprise
   - `RECAPTCHA_API_KEY` - из Credentials
   - `GOOGLE_CLOUD_PROJECT_ID` - ID проекта GCP
7. Перезапустите сервер

**Примечание:** Для разработки и тестирования можно временно отключить reCAPTCHA, не заполняя переменные в `.env`. Сервер автоматически пропустит проверку.
...
- ✅ CORS защита
- ✅ Input sanitization
- ✅ reCAPTCHA Enterprise антибот защита (Google Cloud)
