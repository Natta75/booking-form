...
- **Полное руководство**: [docs/RECAPTCHA_SETUP_GUIDE.md](docs/RECAPTCHA_SETUP_GUIDE.md)
- **Быстрый чек-лист**: [docs/RECAPTCHA_QUICK_CHECKLIST.md](docs/RECAPTCHA_QUICK_CHECKLIST.md)

**Краткая версия:**
1. Перейдите на [reCAPTCHA Admin Console](https://www.google.com/recaptcha/admin/create)
2. Создайте новый сайт:
   - Label: `Booking Form`
   - Тип: **reCAPTCHA v3** ⚠️ ВАЖНО!
   - Домены: `localhost` (для dev) и `yourdomain.ru` (для prod)
3. Получите **Site Key** и **Secret Key**
4. Добавьте ключи в `.env` (раскомментируйте строки)
5. Перезапустите сервер

**Примечание:** Для разработки и тестирования можно временно отключить reCAPTCHA, закомментировав строки `RECAPTCHA_SITE_KEY` и `RECAPTCHA_SECRET_KEY` в `.env`. Сервер автоматически определит отсутствие ключей и пропустит проверку.
